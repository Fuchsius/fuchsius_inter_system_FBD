...
const Tasks = ({ userRole }) => {
  ...
  const sendStatusNotifications = useCallback(async (taskId, taskTitle, status) => {
    ...
    try {
      setLoading(true);
      const [adminsResponse, pmsResponse, taskDetailResponse] = await Promise.all([
        usersAPI.getAll({ role: 'admin', limit: 1000 }),
        usersAPI.getAll({ role: 'pm', limit: 1000 }),
      ...
      if (pmsResponse?.success) {
        pmsResponse.data?.users?.forEach((user) => {
          if (user?.id) recipientIds.add(user.id);
        });
      }

      const projectManagerId = taskDetailResponse?.data?.project?.projectManager?.id;
      if (projectManagerId) {
      ...
      const actorNameRaw = currentUser
        ? `${currentUser.firstName || ''} ${currentUser.lastName || ''}`.trim() || currentUser.email || 'A team member'
        : 'A team member';
      const actorName = actorNameRaw.trim() || 'A team member';

      const message = `${actorName} marked task "${taskTitle || 'Task'}" as ${config.label}.`;
  ...
  const handleMarkComplete = useCallback(async (taskId) => {
    try {
      ...
      await sendStatusNotifications(taskId, task?.title, 'completed');
      setTaskDetails(prev => {
        const updated = { ...prev };
  ...
  }, [fetchTasks, sendStatusNotifications, tasks]);
  ...
  const handleMarkInProgress = useCallback(async (taskId) => {
    try {
      ...
      const task = tasks.find((t) => t.id === taskId);
      await sendStatusNotifications(taskId, task?.title, 'in_progress');
      setTaskDetails(prev => {
        const updated = { ...prev };
        delete updated[taskId];
        return updated;
