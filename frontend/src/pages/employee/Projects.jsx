        ...
        <div>
          ...
          {!loading && (
            <p className="text-xs text-slate-400">
              ...
              {projectDetailsLoading && ' · Loading project teammates…'}
            </p>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {paginatedProjects.length ? (
          ...
          <div className="col-span-full text-center py-12 border border-dashed border-slate-200 rounded-xl">
            <p className="font-medium text-slate-700">{loading ? 'Loading projects...' : 'No tasks assigned yet'}</p>
            <p className="text-sm text-slate-500 mt-1">{loading ? 'Please wait while we fetch your projects' : 'Projects will appear here once they are created.'}</p>
          </div>
        )}
      </div>
