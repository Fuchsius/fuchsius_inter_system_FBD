...
const Profile = ({ userRole }) => {
  ...
  const [avatarUpdatingLoading, setAvatarUpdatingLoading] = useState(false);
  const [profileUpdatingLoading, setProfileUpdatingLoading] = useState(false);
  const [passwordChangingLoading, setPasswordChangingLoading] = useState(false);
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      if (response.success) {
        ...
        return userData; // Return user data for event dispatch
      }
    } catch (error) {
  ...
  const confirmProfilePictureChange = async () => {
    try {
      ...
      setAvatarUpdatingLoading(false);
      setLoading(false);
    }
  };
  ...
  const handleProfileUpdate = async () => {
    try {
      setProfileUpdatingLoading(true);
      setLoading(true);
      ...
      setProfileUpdatingLoading(false);
      setLoading(false);
    }
  };
  ...
  const handlePasswordChange = async () => {
    ...
    if (passwordChangingLoading) return;
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="mt-6 flex justify-end">
          <Button onClick={handleProfileUpdate} disabled={profileUpdatingLoading}>
      ...
      <Card className="p-6">
        ...
        <Button onClick={handlePasswordChange} className="mt-6 bg-[#C4009A] text-white hover:bg-[#7E006C] border-none" disabled={passwordChangingLoading}>
          {passwordChangingLoading ? (
            <>
