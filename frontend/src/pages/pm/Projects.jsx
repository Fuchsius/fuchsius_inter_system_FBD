...
import {
  ...
  Clock, AlertCircle, UserPlus, Trash2, ChevronDown, Eye, Edit, Search
} from 'lucide-react';
...
const mapUserToOption = (user = {}) => {
  ...
  return {
    ...
    originalRole: user.role || '', // Preserve original role for filtering
    email: user.email || '',
    avatar,
...
const Projects = ({ userRole }) => {
  ...
  // Temporarily show all users to debug role filtering
  return users;
}, [users]);
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      // Create notifications for assigned employees (including project manager)
      const assignedUserIds = [...new Set(tasksToCreate.map(t => t.assignedTo))];
      if (currentUserId) {
        assignedUserIds.push(currentUserId);
      }
        ...
        <div className="p-4 border-t border-slate-100 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {(() => {
              ...
              const currentPageNum = currentPaginationPage;
              const maxVisible = 5;
          ...
          <div className="bg-white rounded-xl w-full max-w-full sm:max-w-4xl max-h-[90vh] overflow-y-auto mx-auto">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          ...
                          <div>
                            ...
                            <SearchableDropdown
                              options={newProject.assignedEmployees.map(emp => ({ id: emp.id, name: emp.name }))}
                          ...
                          <div className="flex gap-2">
                            <div className="flex-1">
                              ...
                              <input
                                type="date"
                                value={task.deadline}
                                onChange={(e) => handleUpdateNewProjectTask(task.id, 'deadline', e.target.value)}
                                className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:border-[#C4009A] text-sm"
                              />
                            </div>
                            <button
            ...
            <div className="p-6 border-t border-slate-200 flex justify-end gap-3">
              ...
              <Button
                ...
                disabled={!newProject.title.trim() || newProject.assignedEmployees.length === 0 || savingProject}
              >
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          ...
                          <div>
                            <label className="block text-xs font-medium text-slate-600 mb-1">Assign To</label>
                            <SearchableDropdown
                              options={editingProject.assignedEmployees.map(emp => ({ id: emp.id, name: emp.name }))}
                              value={task.assignedTo || 'all'}
