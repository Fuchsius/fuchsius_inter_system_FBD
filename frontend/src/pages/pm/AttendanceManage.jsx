...
import {
    Users, Calendar, Clock, CheckCircle, XCircle, AlertCircle, Download, Filter, Search, ChevronDown, X, UserPlus, Trash2, Edit, UserX
} from 'lucide-react';
...
const AttendancePage = () => {
    ...
    const fetchAttendance = useCallback(async () => {
        ...
        try {
            const params = {
                limit: 10000, // Fetch more records for client-side filtering
            ...
            if (attendanceRes.success) {
                const formatted = attendanceRes.data.attendance.map(record => {
                    ...
                    return {
                        ...
                        isOnline: record.user?.isOnline || false,
                        date: normalizedDate,
            ...
            if (positionsRes.success) {
                const posList = positionsRes.data.positions?.map((pos) => pos.name) || [];
                setPositions(['all', ...posList]);
            }
        } catch (error) {
    ...
    }, [selectedDate, selectedStatus, startLoading, stopLoading]);
    ...
    const handleUserStatus = useCallback((data) => {
        setAttendanceData(prevData => {
            // Update attendance records for the user
    ...
    }, [displayAttendance, searchTerm, selectedDepartment, selectedStatus, selectedDate, currentUser, selectedPosition]);
    ...
    const handleMarkAllAbsent = useCallback(async () => {
        try {
            ...
            const attendanceRecords = allAttendanceRes.success
                ? allAttendanceRes.data.attendance || []
                : [];

            // Get users who don't have attendance for today
            const usersWithoutAttendance = availableEmployees.filter(emp => {
                ...
                // Only exclude users who already have attendance and the current logged-in user
                const isCurrentUser = currentUser && emp.id === currentUser.id;
                return !hasAttendance && !isCurrentUser;
            });
    ...
    }, [availableEmployees, fetchAttendance, currentUser, startLoading, stopLoading]);
